...
resource "aws_ssm_parameter" "db_endpoint" {
  ...
  depends_on = [time_sleep.wait_for_ssm_update]
}
...
#cool off period, stop tooManyupdates error
#according to chatgpt
resource "time_sleep" "wait_for_ssm_update" {
  
  create_duration = "30s"  # Sleep for 30 seconds
}