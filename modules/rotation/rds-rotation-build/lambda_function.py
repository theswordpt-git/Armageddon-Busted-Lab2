...
def lambda_handler(event, context):
    ...
    logger.info("DEBUG: lambda_handler STARTED")
   ...
   # service_client = boto3.client('secretsmanager', endpoint_url=os.environ['SECRETS_MANAGER_ENDPOINT'])
    ...
    # NEW - Use default (works everywhere)
    #service_client = boto3.client('secretsmanager')

    # Test client creation
    try:
        service_client = boto3.client('secretsmanager')
        logger.info("DEBUG: Client created OK")
    except Exception as e:
        logger.error(f"DEBUG: Client FAILED: {e}")
        raise
...
def set_secret(service_client, arn, token):
    #perplexity debug
    logger.info(f"DEBUG setSecret STARTED for {arn}")  # ADD LINE 1
    ...
    current_dict = get_secret_dict(service_client, arn, "AWSCURRENT")
    #perplexity debug
    logger.info(f"DEBUG: current={current_dict['username']}@{current_dict['host']}:{current_dict['port']}")  # ADD LINE 2


    #perplexity debug
    # ADD THESE DEBUG LINES:
    logger.info(f"DEBUG: Attempting RDS connection to {current_dict['host']}:{current_dict['port']} as {current_dict['username']}")
    ...
    if conn:
        conn.close()
        #perplexity debug
        logger.info("setSecret: AWSPENDING secret is already set")  # ADD LINE 3
...
def connect_and_authenticate(secret_dict, port, dbname, use_ssl):
    ...
    try:
        logger.info(f"DEBUG: Connecting to {secret_dict['host']}:{port} ssl={use_ssl}")
        conn = pymysql.connect(
            ...
            port=port, 
            database=dbname, 
            connect_timeout=10,  # Increased
            ssl=ssl
        )
        logger.info("✅ Connection SUCCESS")
        return conn
    except pymysql.OperationalError as e:
        logger.error(f"❌ Connection FAILED: {e} (code={e.args[0] if e.args else 'unknown'})")
        if 'certificate verify failed' in str(e):
            logger.error("SSL certificate issue - trying without SSL next")
        return None
    except Exception as e:
